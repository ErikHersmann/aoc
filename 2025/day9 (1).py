...
def check_range(check):
    ...
    # Check for a horizontal side: 
        # If 
    # col, row = check
    # check in each row map starting from cur_row - 1 (towards both sides until minrow, maxrow hit) if the col fits in any of the ranges
    # check in each col map starting from cur_col - 1 (towards both sides until mincol, maxcol hit) if the row fits in any of the ranges
    for point in check:
        col, row = point
        found = False
